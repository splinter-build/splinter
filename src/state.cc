...
Node* State::GetNode(std::filesystem::path const& path) {
  // TODO: Costs an allocation.
  // Better would be to have LookupNode use the std::filesystem::equivalent function.
  // But that's not compatible with std::unordered_map.
  std::filesystem::path normalizedPath = path.lexically_normal();
  Node* node = LookupNode(normalizedPath);
  if(node)
  {
    return node;
  }
  node = new Node(std::move(normalizedPath));
...
Node* State::SpellcheckNode(std::filesystem::path const& path) {
  ...
  {
    int distance = EditDistance(pathKey.generic_string().c_str(), path.generic_string().c_str(), kAllowReplacements, kMaxValidEditDistance);
...
bool State::AddDefault(std::filesystem::path const& path, std::string* err) {
  ...
  if (!node) {
    *err = string_concat("unknown target '", path.generic_string().c_str(), "'");
    return false;
  }
